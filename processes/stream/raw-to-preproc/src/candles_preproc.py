...
class CandlesPreproc(PreprocBase):
    ...
    async def _aggregate(self, raw_messages: []) -> dict:
        ...
        df_1min.index = pd.to_datetime([msg["ts"] for msg in raw_messages].copy(), unit="ms")

        df_1min["close_time"] = df_1min.index.ceil("1min")
        df_1min["open_time"] =  df_1min["close_time"] - pd.Timedelta(minutes=1)
        df_1min["vol"] = df_1min["amount"]
        df_1min = df_1min[["open_time", "close_time", "open", "high", "low", "close", "vol"]]
        df_1min_resampled = df_1min.resample("1min", label="right", closed="right").agg("last")
